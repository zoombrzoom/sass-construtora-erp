...
import { deleteContaPagar, getContasPagar } from '@/lib/db/contasPagar'
...
export default function ContasPessoaisPage() {
  ...
  inicioPadrao.setDate(hoje.getDate() - 30) // 31 dias incluindo hoje
  ...
  const loadContas = async () => {
    try {
      const data = await getContasPagar({
        includeParticular: permissions.canAccessContasParticulares,
      })
      setContas(data.filter((c) => Boolean(c.pessoal) || c.obraId === 'PESSOAL'))
    } catch (error) {
    ...
    <div>
      ...
      <div className="bg-dark-500 border border-dark-100 rounded-xl p-4 mb-4">
        ...
        <div className="mt-3 pt-3 border-t border-dark-100 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => setFiltros((p) => ({ ...p, dataInicio: toInputDate(inicioPadrao), dataFim: toInputDate(hoje) }))}
